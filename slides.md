...
Finding an optimal schedule is an NP-complete problem, where a brute-force solution is $$O(n!)$$.
...
```scala
def fancyWordCount(words: List[String]): Map[String, Int] = {
  words
    filter { _.punctuation }
    map { _.toLowercase }
    groupBy { identity }
    mapValues { _.length }
}
...
![inline](images/graphviz/sparselu-sample.png)
